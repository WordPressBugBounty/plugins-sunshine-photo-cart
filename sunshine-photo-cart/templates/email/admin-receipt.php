...
<div id="order-actions">
	<a href="<?php echo esc_url( admin_url( 'post.php?action=edit&post=' . $order->get_id() ) ); ?>" class="button"><?php esc_html_e( 'View Order', 'sunshine-photo-cart' ); ?></a></a>
	<a href="<?php echo esc_url( admin_url( 'post.php?sunshine_invoice=1&post=' . $order->get_id() ) ); ?>" class="button"><?php esc_html_e( 'View Invoice', 'sunshine-photo-cart' ); ?></a></a>
</div>
		...
		<?php foreach ( $cart as $cart_item ) { ?>
			<tr class="order-item <?php echo esc_attr( $cart_item->classes() ); ?>">
				...
				<td class="order-item--data">
					...
					<div class="order-item--gallery"><?php echo wp_kses_post( $cart_item->get_gallery_hierarchy() ); ?></div>
					<div class="order-item--image-name"><?php echo wp_kses_post( $cart_item->get_image_name( 'filename' ) ); ?></div>
...
if ( ! empty( $file_names ) ) {
	?>
<div id="order-files">
	<h3><?php esc_html_e( 'Files', 'sunshine-photo-cart' ); ?></h3>
	<?php
	$file_names = array_unique( $file_names );
	asort( $file_names );
	echo esc_html( join( ', ', $file_names ) );
	?>
</div>
<?php } ?>
